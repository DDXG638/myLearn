...
- (void)flushMessageQueue:(NSString *)messageQueueString{
    ...
    // 通过WebViewJavascriptBridge._fetchQueue()获取事件队列中的多有事件
    ...
    for (WVJBMessage* message in messages) {
        ...
        // 如果有message.responseId，获取responseData之后执行回调函数，并且删除message.responseId
        if (responseId) {
            WVJBResponseCallback responseCallback = _responseCallbacks[responseId];
            responseCallback(message[@"responseData"]);
            [self.responseCallbacks removeObjectForKey:responseId];
        } else {
            // 如果没有message.responseId，
...
// 返回数据给web，其实就是调用window.WebViewJavascriptBridge._handleMessageFromObjC方法
